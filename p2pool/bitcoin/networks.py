...
nets = dict(
    ...
    myria_scrypt=math.Object(
        ...
        SYMBOL='MYR',
        CONF_FILE_FUNC=lambda: os.path.join(os.path.join(os.environ['APPDATA'], 'myriadcoin') 
		if platform.system() == 'Windows' else os.path.expanduser('~/Library/Application Support/myriadcoin/') 
		if platform.system() == 'Darwin' else os.path.expanduser('~/.myriadcoin'), 'myriadcoin.conf'),
        BLOCK_EXPLORER_URL_PREFIX='http://myriad.theblockexplorer.com/block_crawler.php?block_hash=',
        ADDRESS_EXPLORER_URL_PREFIX='http://myriad.theblockexplorer.com/block_crawler.php?address=',##not supported by crawler
        TX_EXPLORER_URL_PREFIX='http://myriad.theblockexplorer.com/block_crawler.php?transaction=',
        SANE_TARGET_RANGE=(2**256//1000000000 - 1, 2**256//1000 - 1),
        DUMB_SCRYPT_DIFF=2**16,
        DUST_THRESHOLD=0.0001,
    ),
    myria_qubit=math.Object(
        P2P_PREFIX='af4576ee'.decode('hex'), #pchmessagestart
        P2P_PORT=10888,
        ADDRESS_VERSION=50, #pubkey_address
        RPC_PORT=10889,
	...
	BLOCKHASH_FUNC=lambda data: pack.IntType(256).unpack(__import__('qubit_hash').getPoWHash(data)), #we need it?
        BLOCK_PERIOD=30, # seconds
        SYMBOL='MYR',
        CONF_FILE_FUNC=lambda: os.path.join(os.path.join(os.environ['APPDATA'], 'myriadcoin') 
		if platform.system() == 'Windows' else os.path.expanduser('~/Library/Application Support/myriadcoin/') 
